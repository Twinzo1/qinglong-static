...
let EnvService = class EnvService {
    constructor(logger) {
        this.logger = logger;
        this.cronDb = new nedb_1.default({ filename: config_1.default.envDbFile });
    ...
    async disabled(ids) {
        return new Promise((resolve) => {
            this.cronDb.update({ _id: { $in: ids } }, { $set: { status: env_1.EnvStatus.disabled } }, { multi: true }, async (err) => {
                await this.set_envs();
                resolve();
            });
        });
    ...
    async enabled(ids) {
        return new Promise((resolve) => {
            this.cronDb.update({ _id: { $in: ids } }, { $set: { status: env_1.EnvStatus.normal } }, { multi: true }, async (err, num) => {
                await this.set_envs();
                resolve();
            });
        });
    ...
    async updateNames({ ids, name }) {
        return new Promise((resolve) => {
            this.cronDb.update({ _id: { $in: ids } }, { $set: { name } }, { multi: true }, async (err, num) => {
                await this.set_envs();
                resolve();
            });
        });
